...
var HttpServer = function(callback) {
  ...
  self.fork = function(workers) {
    if (syscalls.fork() == 0) {
      console.log("In the child process, PID=" + syscalls.getpid());
      self.start();
    } else {
      console.log("In the master process, PID=" + syscalls.getpid());
      if (--workers > 0) self.fork(workers);
    }
  };
};
